...
export function localDateTimeToUtc(value, zone) {
  ...
  for (let i = 0; i < 5; i++)
    utc +=
      wanted -
      Date.parse(formatLocalInput(new Date(utc).toISOString(), zone) + ":00Z");
...
export function statusUtilization(events, center, from, to, now = Date.now()) {
  const history = events
    .filter((e) => e.work_center_id === center)
    .sort((a, b) => Date.parse(a.created_at) - Date.parse(b.created_at));
  const start = Date.parse(from),
    end = Math.min(Date.parse(to), now);
  let running = 0,
    observed = 0;
  for (let i = 0; i < history.length; i++) {
    const a = Math.max(start, Date.parse(history[i].created_at)),
      b = Math.min(
        end,
        i + 1 < history.length ? Date.parse(history[i + 1].created_at) : end,
      );
    if (b > a) {
      observed += b - a;
      if (history[i].new_status === "running") running += b - a;
    }
  }
  return observed
    ? {
        percent: (running / observed) * 100,
        observedMinutes: observed / 60000,
        runningMinutes: running / 60000,
        coverage: end > start ? observed / (end - start) : 0,
      }
    : null;
}
