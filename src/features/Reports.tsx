...
export default function Reports({
  ...
  const [period, setPeriod] = useState("today"),
    ...
    [grouping, setGrouping] = useState("category");
  ...
  const events = (s.tables.downtime_events || []).filter(
    (e) =>
      ...
      (reason === "all" ||
        e.reason_id === reason ||
        e.sub_reason_id === reason) &&
      downtimeMinutes(e, range.from, range.to) > 0,
  );
  const minutes = (e: Row) => downtimeMinutes(e, range.from, range.to);
  const grouped = pareto(events, minutes, (e: Row) =>
    String(
      grouping === "reason" ? e.sub_reason_id || e.reason_id : e.reason_id,
    ),
  );
  ...
  async function exportCsv() {
    ...
    try {
      ...
      if (!r.ok) {
        setExportError((await r.json()).error || "error");
        return;
      }
  ...
  const observations = (s.tables.oee_observations || []).filter(
    (o) =>
      centers.some((c) => c.id === o.work_center_id) &&
      String(o.started_at) >= range.from &&
      String(o.ended_at) <= range.to,
  );
  const output = (s.tables.production_entries || []).filter(
    (e) =>
      !invalidRange &&
      centers.some((c) => c.id === e.work_center_id) &&
      String(e.created_at) >= range.from &&
      String(e.created_at) < range.to,
  );
  const activity = (s.tables.status_events || []).filter(
    (e) =>
      !invalidRange &&
      centers.some((c) => c.id === e.work_center_id) &&
      String(e.created_at) >= range.from &&
  ...
  const orders = (s.tables.production_orders || []).filter(
    (o) =>
      !invalidRange &&
      (line === "all" || o.line_id === line) &&
      ((area === "all" && machine === "all") ||
        centers.some((c) => c.order_id === o.id) ||
        output.some((e) => e.order_id === o.id)) &&
      (!o.start_time || String(o.start_time) < range.to) &&
      (!o.expected_finish ||
        String(o.expected_finish) >= range.from ||
        output.some((e) => e.order_id === o.id)),
  );
    ...
    <>
      <section className="panel report-controls">
        ...
        {can("reports", "export") && (
          ...
          >
            {t(exporting ? "loading" : "exportDowntime")}
          </button>
        )}
      </section>
      {invalidRange && (
      ...
      {(report === "lineReport" || report === "dailySummary") && (
        <section className="panel">
          <h3>{t("lineProduction")}</h3>
          {(s.tables.production_lines || [])
            ...
            .map((l) => {
              ...
              return (
                <div className="attention-row" key={String(l.id)}>
                  <strong>{localName(l, lang)}</strong>
                  <span>
                    {t("produced_quantity")}:{" "}
                    {entries.reduce((n, e) => n + Number(e.produced), 0)}
                  </span>
                  <span>
                    {t("rejected_quantity")}:{" "}
                    {entries.reduce((n, e) => n + Number(e.rejected), 0)}
                  </span>
                </div>
              );
            })}
      ...
      {(report === "utilization" || report === "dailySummary") && (
        <>
          ...
          <section className="panel">
            <h2>{t("utilization")}</h2>
            <p className="muted">{t("utilizationHelp")}</p>
            {centers.map((c) => {
              const u = statusUtilization(
              ...
              return (
                <div className="attention-row" key={String(c.id)}>
      ...
      {report === "dailySummary" && (
        <section className="panel">
          <h2>{t("periodOutput")}</h2>
          <div className="summary-strip">
            <span>
              {t("produced_quantity")}:{" "}
              <b>{output.reduce((n, e) => n + Number(e.produced), 0)}</b>
            </span>
            <span>
              {t("rejected_quantity")}:{" "}
              <b>{output.reduce((n, e) => n + Number(e.rejected), 0)}</b>
            </span>
            <span>
              {t("activeOrders")}:{" "}
              <b>{orders.filter((o) => o.status === "active").length}</b>
            </span>
          </div>
        </section>
      )}
        ...
        <section className="panel">
          ...
          {activity.map((e) => (
            <div className="attention-row" key={String(e.id)}>
              <strong>
                {String(
                  s.tables.memberships?.find((m) => m.user_id === e.created_by)
                    ?.display_name || "—",
                )}
              </strong>
              <span>
